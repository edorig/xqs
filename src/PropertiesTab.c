 ...
 * Description: PropertiesTab.c -- procedures and functions used for the 
 *	Properties tab in the xqs program, includes code derived from xprop
 *****
  *****/
...
void xprop( Window, Bool, Bool );

Widget prop_out;

void PropertiesTabActive()
{
  Window win;
...
void xprop( win, show_type, ignore_frame  )
  Window win;
  Bool show_type, ignore_frame;
{
  Display *dpy = info_dpy;
  thunk *props, *Handle_Prop_Requests();
  Window root;
  int dummyi, frame_only;
  unsigned int dummy;
  int (*old_handler)();

  target_win = win;
  notype = !show_type;
  frame_only = !ignore_frame;
  
  BAD_WINDOW = FALSE;
  old_handler = XSetErrorHandler(mal_window_handler);

  Setup_Mapping();
  if( ! frame_only ) {
       if( XGetGeometry( dpy, target_win, &root, &dummyi, &dummyi, &dummy, 
			 &dummy, &dummy, &dummy) && target_win != root ) {
	    XSync( dpy, False );
  ...
  if( BAD_WINDOW ) return;

  props = Handle_Prop_Requests( 0, NULL );

}

/* handle error in xprop */
int mal_window_handler(disp, err)
    Display *disp;
    XErrorEvent *err;
{
    char badid[20];
    ...
    sprintf(badid, "0x%x", (unsigned int)err->resourceid);
...
thunk t;
{
  ...
  if (!list)     Fatal_Error("Out of memory!");
     ...
     FILE *stream;
{
	...
	c = getc(stream);
	if (c==EOF)    Fatal_Error("Bad format file: Unexpected EOF.");
	return(c);
}
...
#define E_DFORMAT " = $0+"         /* Display pattern for empty properties */
...
void
Apply_Empty_Formats(format, dformat)
     char **format;
     char **dformat;
{
  if (!*format)
     ...
     char *dformat;
{
	...
	switch (c) {
	      case 'n':
		/* putchar('\n'); */
		XmTextNewLine( prop_out );
		break;
	      case 't':		
		/* putchar('\t'); */
		XmTextAppend( prop_out,  "\t" );
		break;
	      case '0':
	      case '1':
	      case '2':
		...
		/* putchar((int) i); */
		XmTextPrintf( prop_out, "%d", (int)i );
		break;
	      default:
		/* putchar(c); */
		XmTextPrintf( prop_out, "%c", c );
		break;
	}
	return(dformat);
}
...
{
  char *tmp;
  tmp = XtMalloc( strlen(str1)+strlen(str2)+1 );
  tmp = strcpy( tmp, str1 );
  tmp = strcat( tmp, str2 );
  return tmp;
}
     ...
     char *format;
{
	...
	if( dformat[0]=='+' ){
	     ...
	     for( ; i<thunks->thunk_count; i++ ){
		  if ( i == thunks->thunk_count-1 )
		       XmTextPrintf( prop_out, "%s",
				     Format_Thunk_I(thunks, format, (int) i));
		  else
		       XmTextPrintf( prop_out, "%s, ",
				     Format_Thunk_I(thunks, format, (int) i) );
	     }
	} else  XmTextPrintf( prop_out, "%s",
			      Format_Thunk_I(thunks, format, (int) i));
     ...
     char *format, *dformat, *prop;
{
  ...
  if( prev_length > 1 && length < 1 )  XmTextNewLine(prop_out);
  
  if( length < 1 )
       XmTextPrintf( prop_out, "%s", prop);
  else XmTextPrintf( prop_out, "\n%s", prop);
    
  if( !atom ) {
       XmTextPrintf( prop_out, ":  no such atom on any window.\n");
       return;
  }
  
  if( !size ) {
       XmTextAppend( prop_out, ":  not found.");
       return;
  }
  
  if( !notype && type )
       XmTextPrintf( prop_out, "(%s)", Format_Atom(type));
  ...
  if( length < 1 )
       Apply_Empty_Formats( &format, &dformat );
  else Apply_Default_Formats(&format, &dformat);

  fsize=Get_Format_Size(format);
  if( fsize != size && fsize != 0 ) {
       XmTextPrintf( prop_out, ": Type mismatch: assumed size %d bits, actual size %d bits.\n",
		     fsize, size);
       return;
  }
  ...
  Display_Property(thunks, dformat, format);
  if( length < 1 )  XmTextNewLine( prop_out );
}
